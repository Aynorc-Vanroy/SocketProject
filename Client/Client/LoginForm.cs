        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            Thread tdLogining = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        WriteToolStripMsg(msg, Color.Black);
                        return;
                    }
                    //获取控件上输入的登录用户和密码
                    ...
                    if (onlineClients.ConvertToEntityList<OnlineClient>().Exists(s => s.UserID.ToLower() == _LoginID.ToLower()))
                    {
                        ...
                        WriteToolStripMsg(message, Color.Black);
                    }
                    else
                    {                        
        ...
        private void InitLoading()
        {
            Thread t = new Thread(() =>
            {
                ...
                try
                {
                    ...
                    {
                        WriteToolStripMsg(string.Format("Web服务请求拒绝:{0}", web_Result), Color.Black);
                        blFinish = false;
                        return;
                    }
                ...
                {
                    WriteToolStripMsg("系统出错,请联系技术支持!", Color.Black);
                    this.BeginInvoke(new MethodInvoker(() =>
                    {
                        this.btnLogin.Enabled = false;
                    }));
